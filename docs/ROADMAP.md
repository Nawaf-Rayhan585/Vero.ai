...
**Phases 0-7 are accepted by the owner.** **Phase 8 (vehicle detection/counting + OCR + QR + barcode, with the per-camera module selection they need) is implemented and awaiting owner verification** — it is not marked complete until the owner has verified it (Development Rule 6). Phase 9 has not started.

### What exists today

- `backend/` — FastAPI + SQLAlchemy 2 (sync) + Alembic + PostgreSQL 16. Layout: `main.py` and `app/` (`config`, `database`, `models`, `schemas`, `crypto`, `detection_runner`, `camera_testing`, `tracking`, `counting`, `zones`, `heatmap`, `modules`, `scanning`, `routers/jobs`, `routers/cameras`, `routers/tracking`, `routers/lines`, `routers/zones`). Endpoints: `GET /health`, `GET /health/db`, `POST/GET /jobs`, `GET /jobs/{id}`, full `/cameras` CRUD, `POST /cameras/{id}/test-connection`, `GET /cameras/{id}/snapshot`, `/cameras/{id}/tracking/{start,stop,status,latest-frame}` (`latest-frame?heatmap=true` returns the same annotated frame with the accumulated heat blended on), `/cameras/{id}/lines` (create/list) + `DELETE /lines/{id}`, and `/cameras/{id}/zones` (create/list) + `DELETE /zones/{id}`. Four tables (`jobs`, `cameras`, `lines`, `zones`) and five migrations (`cameras.enabled_modules` — which AI modules that camera runs, default people-only — is the latest) — `lines` and `zones` cascade on camera delete; a zone's polygon is a JSONB list of normalized points. Tracking sessions (live in/out counts for people and vehicles, live zone occupancy, the heat grid, what the QR/barcode/OCR modules have read) are **not** a table; fully in-memory, one background thread per actively-tracked camera, reset every time tracking starts. Camera passwords are encrypted at rest and never returned by the API. 245 automated tests (pytest, real PostgreSQL, real YOLO/ByteTrack inference, real OpenCV) live in `backend/tests/`.
...
- **Reads are ephemeral and deduplicated per session.** A QR/barcode/text value is one entry with a sighting count (a new sighting only after a 5 s gap), the 50 most recent are kept, and nothing is stored — that is Phase 9.
...
Owner verification of Phase 8, then the explicit instruction **"START PHASE 9"**.
